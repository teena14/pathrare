...
<b>20% Semantic Vector Search</b> — symptom text similarity against disease embeddings</p>
          ...
          <motion.div key="input" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} className="space-y-6">
            ...
            {loading && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white rounded-3xl border border-surface-200 p-6 space-y-4">
                ...
                {['OCR — Extracting text from document', 'Gemini — Identifying clinical symptoms', 'Generating symptom embeddings', 'Matching against Orphanet disease dataset'].map((step, i) => (
